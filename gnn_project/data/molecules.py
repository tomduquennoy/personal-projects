...
# Compatibility fix for loading old DGL pickle files
# Inject DGLHeteroGraph into dgl.heterograph module for backward compatibility
import sys
import dgl.heterograph
if not hasattr(dgl.heterograph, 'DGLHeteroGraph'):
...
class MoleculeDGL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for molecule in self.data:
            ...
            g = dgl.graph((src_nodes, dst_nodes), num_nodes=molecule['num_atom'])
...
class MoleculeAqSolDGL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for molecule in self.data:
            ...
            self.graph_lists.append(g)
            self.graph_labels.append(molecule[3])
...
def self_loop(g):
    ...
    # Combine non-self edges with self loops
    new_src = np.concatenate([src[non_self_edges_idx], nodes])
    new_dst = np.concatenate([dst[non_self_edges_idx], nodes])
    
    new_g = dgl.graph((new_src, new_dst), num_nodes=g.number_of_nodes())
